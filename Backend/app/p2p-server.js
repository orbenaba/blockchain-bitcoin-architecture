...
const { CurrentActiveModel } = require("./CurrentActive");
...
class P2pserver {
  ...
  async listen() {
    ...
    console.log(
      `Listening for peer to peer connection on port: ${this.P2P_PORT}`
    );
  }
  ...
  async connectionToPeers() {
    ...
    this.peers.forEach(async (peer) => {
      ...
      const p = Number(peer.slice(15, 20));
      if (p !== this.P2P_PORT) {
        try {
          ...
          if (status === "open") {
            ...
            await socket.on("open", async () => {
              await this.connectionSocket(socket);
            });
