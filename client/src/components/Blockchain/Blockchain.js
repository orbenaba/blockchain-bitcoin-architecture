...
export default function Blockchain() {
  ...
  const [TXPerBlock, setTXPerBlock] = useState(0);
  ...
  useEffect(async () => {
    ...
    if (typeof res.data.difficulty !== "undefined") {
      ...
      setTXPerBlock(res.data.TXPerBlock);
      //check out the miner
  ...
  if (loading) {
    return <Loader flag={flag}></Loader>;
  }

  let form = (
    <form onSubmit={onSubmit} className="general">
      <div style={{ position: "relative", left: "-10rem" }}>
        <DropdownButton
          ...
          title={
            fromAddress === "" ? (
              <span className="dropdown-title">From Address</span>
            ) : (
              <span style={{ backgroundColor: "grey", color: "black" }}>
                0x{fromAddress.slice(1, 40)}
              </span>
            )
          }
        ...
        >
          {userOptions.map((u) => {
            return (
              ...
              >
                {u.name + " - 0x" + u.publicKey.slice(1, 30) + " ..."}
              </Dropdown.Item>
            );
          })}
        </DropdownButton>
      ...
      <div style={{ position: "relative", right: "-10rem" }}>
        <DropdownButton
          ...
          title={
            toAddress === "" ? (
              <span className="dropdown-title">To Address</span>
            ) : (
              <span style={{ backgroundColor: "grey", color: "black" }}>
                0x{toAddress.slice(1, 40)}
              </span>
            )
          }
          id="dropdown-item-button"
  ...
  let newBlockchain = (
    <form onSubmit={onCreate} className="general">
      <input
        type="number"
        value={difficulty}
        name={difficulty}
  ...
  let deleteButton = (
    <button
      type="submit"
      onClick={() => {
        if (window.confirm("Are you sure you wish to delete the blockchain?"))
          deleteBlockchain();
      }}
      style={{
        position: "absolute",
    ...
    >
      ...
      <span>
        <strong>Delete all the blockchain</strong>
      </span>
    </button>
  );
  ...
  if (difficulty !== 0 && pendingTransactions.length >= 3) {
        ...
        <div className="text-center">
          ...
          <div className="general">
            ...
            >
              ...
              ></span>
              <span>
                <strong>Mine 4 TXs in a shout (One for the miner)</strong>
              </span>
            </button>
            <br></br>
            {deleteButton}
        ...
        <div className="text-center">
          ...
          <div className="general">
            <Recap
              ...
              pendingTransactionsLength={pendingTransactions.length}
            ></Recap>
            <Link to="/miner" style={{ position: "relative", left: "40px" }}>
              Click here to create a miner
            </Link>
            <br></br>
            {deleteButton}
          </div>
    ...
    return (
      <div className="general text-center">
        ...
        <button
          ...
          onClick={() => {
            if (
              window.confirm("Are you sure you wish to delete the blockchain?")
            )
              deleteBlockchain();
          }}
          style={{
    ...
    return (
      <div className="text-center">
        ...
        >
          Total pending TXs: {pendingTransactions.length}
        </h4>
      </div>
    );
  }
