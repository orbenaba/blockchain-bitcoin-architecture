...
export default function Miner() {
  ...
  useEffect(async () => {
    axios.get("http://localhost:4000/miner").then((res) => {
  ...
  async function deleteMiner(e) {
    await axios.delete("http://localhost:4000/miner").then((res) => {
      setPublicKey("");
    });
  }
  ...
  async function onSubmit(e) {
    ...
    await axios
      .post("http://localhost:4000/miner", data)
      .then((res) => {
        setPublicKey(res.data.publicKey);
        setPrivateKey(res.data.privateKey);
        setMoney(res.data.money);
  ...
  if (loading) {
    return <Loader flag={false}></Loader>;
  }
      ...
      <div>
        ...
        >
          <div className="form-group">
            <label for="Money" className="control-label col-sm-3">
              Money:
            </label>
          ...
          <div className="form-group">
            <label for="publicKey" className="control-label col-sm-3">
              Public Key:
            </label>
            <div
          ...
          <button
            type="button"
            onClick={() => {
              if (window.confirm("Are you sure you wish to delete the miner?"))
                deleteMiner();
            }}
